...
def main():
    ...
    for plugin in plugins:
        ...
        if plugin.get("disabled", False):
            if os.path.exists(plugin_output):
                print(f"🗑️  Removendo plugin desabilitado: {plugin_path}")
                shutil.rmtree(plugin_output)
            continue

        print(f"📦 Processando plugin: {package}")
    ...
    print("✅ Dynamic plugins processados com sucesso.")
